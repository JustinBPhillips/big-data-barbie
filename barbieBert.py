...
# Readme:
# This code is used to parse and categorize all references to the (case-insensitive) keyword 'Barbie' on Facebook (facebook.csv) and Instagram (instagram.csv)
# That material was extracted from CrowdTangle, between July 22 and August 22.
# Users looking to replicate these findings will need to use CrowdTangle (or the Meta Content Library) to extract that data, as it is owned by Meta.
# The R code (BertBarbie.R) is used to create the graphs and the more detailed results found in the Online Supplement.


homedir="/home/justin/OneDrive/manuscripts/Barbie/bert/tmp/"

facebook = pd.read_csv(homedir+"facebook.csv", sep="\t", keep_default_na=False)
facebook['UNIXTIME'] = [round(dateparser.parse(item).timestamp()) for item in facebook['Post Created']]
insta = pd.read_csv(homedir+"instagram.csv", sep="\t", keep_default_na=False)
...
combo = pd.DataFrame()
combo['DATE'] = list(itertools.chain(facebook['UNIXTIME'],insta['UNIXTIME']))
combo['TEXT'] = list(itertools.chain(facebook['ALLPOSTTEXT'],insta['ALLPOSTTEXT']))
combo['PLATFORM'] = list(itertools.chain(list(itertools.repeat("Facebook", facebook.shape[0])),list(itertools.repeat("Instagram", insta.shape[0]))))


combo.to_csv(homedir+'all_barbie_posts_combo.tsv', index=False, sep='\t', header=False, quoting=csv.QUOTE_NONE)
...
npUniqueSentences.to_csv(homedir+'uniques_barbie.tsv', index=False, sep='\t', header=False, quoting=csv.QUOTE_NONE)

pd.DataFrame({'index':indices,'sentence':allSentences}).to_csv(homedir+'all_sentences_barbie.tsv', index=False, sep='\t', quoting=csv.QUOTE_NONE)
...
def send2HDBSCAN(umap_file,cluster_file,min_cluster_percentage):
	...
	hdbscan_model = HDBSCAN(min_samples=50,min_cluster_size=min_cluster_size, cluster_selection_method='eom', prediction_data=False)
...
send2UMAP(homedir+"embedding_barbie.pbz2",homedir+"umap_barbie.tsv")
send2OpenTSNE(homedir+"umap_barbie.tsv",homedir+"tsne_barbie.tsv")

min_cluster_percentage = 0.0005
send2HDBSCAN(homedir+"tsne_barbie.tsv",homedir+"hdbscan_barbie"+str(min_cluster_percentage)+"_mcp_50ms.tsv",min_cluster_percentage)
barbie_BERTopic = send2BERTopic(homedir+"uniques_barbie.tsv",homedir+"tsne_barbie.tsv",homedir+"hdbscan_barbie0.0005_mcp_50ms.tsv")
results2browser(resultsFromBERT2HDBSCANtopics(barbie_BERTopic,homedir+"hdbscan_barbie0.0005_mcp_50ms.tsv"))
results2csv(resultsFromBERT2HDBSCANtopics(barbie_BERTopic,homedir+"hdbscan_barbie0.005_mcp.tsv"),homedir+"results_hdbscan_barbie0.005_mcp.tsv")
